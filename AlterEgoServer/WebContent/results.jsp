...
<%@page import="org.cl.nm417.data.Profile"%><html xmlns="http://www.w3.org/1999/xhtml">
<head>
...
<body>
	...
	<%
		...
		for (String rerank: reranks){
			for (Boolean b1: bvalues){
				for (Boolean b2: bvalues){
					...
					method.put("method",rerank); method.put("interleave",false);
					method.put("interleaveMethod","teamdraft"); 
					method.put("lookatrank", b1);
					method.put("visited", b2); method.put("visitedW", 10);
					methods.add(method);
				}
			}	
		}
		...
		for (Boolean b1: bvalues){
			HashMap<String, Object> method = new HashMap<String, Object>();
			method.put("method","pclick"); method.put("interleave",false);
			method.put("interleaveMethod","teamdraft"); 
			method.put("lookatrank", true);
			method.put("visited", b1); method.put("visitedW", 10);
			methods.add(method);
		}
		...
		if (userid != null && userid.length() > 0){
			...
			for (String title: titles){
				...
				out.println(Evaluation.getRow(topColumn,true, null));
					...
					if (snippet == null){
						...
						for (String file: files){
							...
							for (HashMap<String, Object> m: methods){
								...
								results = AlterEgo.finalSearchGoogle(query, profile, (String)m.get("method"), (Boolean)m.get("interleave"), (String)m.get("interleaveMethod"), (Boolean)m.get("lookatrank"), (Boolean)m.get("visited"), (Integer)m.get("visitedW"), results);
						...
						out.println(Evaluation.getRow(scores, false, "gradeA"));
						out.flush();
				...
				out.println(Evaluation.getRow(avg, true,"gradeA"));
				out.flush();
				...
				out.println(Evaluation.getRow(impr, true,"gradeA"));
				out.flush();
				...
				out.println(Evaluation.getRow(ttest, true,"gradeA"));
				out.println(Evaluation.getRow(ttest2, true,"gradeA"));
				out.flush();
			...
			out.println(Evaluation.getRow(users, true, null, "th"));
			...
			for (String cProfile: profiles){
				...
				if (cProfile.equals("Google")){
					...
					for (String cuserid: usrs){
						...
						userid = cuserid;
						File f = new File("/Users/nicolaas/Desktop/AlterEgo/dataprocessing/data/profiles/" + userid + "/" + userid + "_b_n_n_n_y_n_n_n_n.txt");
						...
						if (f.exists()){
							...
							for (String query: mqueries.get(userid)){	
								...
								results = AlterEgo.finalSearchGoogle(query, mprofiles.get(userid), "lm", false, null, true, false, 10, results);
					...
					for (String cuserid: usrs){
						...
						userid = cuserid;
						File f = new File("/Users/nicolaas/Desktop/AlterEgo/dataprocessing/data/profiles/" + userid + "/" + userid + "_" + cProfile + ".txt");
						...
						if (f.exists()){
							...
							for (String query: mqueries.get(userid)){	
								...
								results = AlterEgo.finalSearchGoogle(query, mprofiles.get(userid), (String)m.get("method"), (Boolean)m.get("interleave"), (String)m.get("interleaveMethod"), (Boolean)m.get("lookatrank"), (Boolean)m.get("visited"), (Integer)m.get("visitedW"), results);
								String calcScore = Evaluation.calculateDCG(snippets, results, 10, true);
								Double d = Double.parseDouble(calcScore);
								//System.out.println(query + " => " + d);
								cscores.add(d);
				...
				out.println(Evaluation.getRow(fields, false,"gradeA","td"));
				out.flush();
