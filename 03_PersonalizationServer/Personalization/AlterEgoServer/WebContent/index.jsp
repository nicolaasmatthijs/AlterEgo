...
/**
 * Top part of the main personalization User Interface. This part allows you to select the
 * different parameters that should be used and generate a profile. You can then issue a query
 * and re-rank the results using that profile and the selected re-ranking parameters
 */
 -->
...
<body>
	...
	<div style="float:left; width:50%; border-right: 1px solid #aaa; padding: 10px 0px; height: 170px;" id="generateDiv">
				...
				<tr style="height:25px; vertical-align:top;">
					<td colspan="3">
						<select id="user" style="width: 150px;">
							<!-- List of users that have installed the add-on -->
		...
		<div style="float: left; width: 33%">
			<table>
				...
				<tr>
					<!-- Make the weight for a term dependent on the total number of terms available
						in the datasource it comes from -->
					<td class="inputCell">
		...
		<div style="float: left; width: 33%">
			<table>
				...
				<tr>
					<!-- Load the Google N-Gram corpus into memory -->
					<td colspan="2">
	...
	<script language="JavaScript" type="text/javascript">
		...
		// Disable all filtering methods is Google N-Gram filtering is checked
		...
		// Disable all Part of Speech filtering methods if All POS is checked
		...
		$("#generate_profile").bind("click", function(ev){
			...
			// Submit this parameter configuration to the user profile generation phase
			var querystring = "";
			for (var key in obj){
		...
		$("#search").bind("click", function(ev){
			...
			// Profile chosen to use for re-ranking
			var profilename = $("#profilename").val();

			// Do the actual search and re-rank the results. Display the results on screen
		...
		// Google N-Gram corpus loading
		$("#loadNGram").bind("click", function(ev){
			parent.output.location.href = "loadNGram.jsp";
			parent.output2.location.href = "about:blank";
		});

		// Load the list of profiles available for the selected user
